...
void ColorTable::setRandomColor( const int& max_color_value, const int& position ) {
    ...
    mVector[position].setRed(std::rand());
    mVector[position].setGreen(std::rand());
    mVector[position].setBlue(std::rand());
}
...
void ColorTable::insertGradient( const Color& color1, const Color& color2, const int& position1, const int& position2 ) {
    ...
    redslope = double(color2.getRed() - color1.getRed()) / double(colors - 1);
    greenslope = double(color2.getGreen() - color1.getGreen()) / double(colors - 1);
    blueslope = double(color2.getBlue() - color1.getBlue()) / double(colors - 1);

    for (int i = 0; i < colors; i++) {
        this->mVector[i].setRed(color1.getRed() + i * redslope);
        this->mVector[i].setGreen(color1.getGreen() + i * greenslope);
        this->mVector[i].setBlue(color1.getBlue() + i * blueslope);
    }
}
